...
public class CustDashboardPage extends LoadableComponent<CustDashboardPage> {
   ...
   * 
   */
  public void enterEffectiveDate(HashMap<String, String> testdata) throws Exception {
    try {
      String effectDate = GenericUtils.setDate(testdata.get("Event"),
          Integer.parseInt(testdata.get("noofDays")), Integer.parseInt(testdata.get("noofYear")));
  ...
  public void selectReinstateReason(HashMap<String, String> testdata, boolean screenshot,
      ExtentTest extentedReport) throws Exception {
    try {
      WaitUtils.waitForElement(driver, drpReinstateReason, 5);
  ...
  /**
   * Validate the Cancel Policy date
   * 
   * @param extentedReport
   * @throws Exception
   * 
   */
  public void validateCancelDate(HashMap<String, String> testdata, ExtentTest extentedReport)
      throws Exception {
    ...
    try {
      ...
      (new WebDriverWait(driver, 180).pollingEvery(200, TimeUnit.MILLISECONDS)
          ...
          .withMessage("Unable to find Cancel Policy"))
              .until(ExpectedConditions.elementToBeClickable(reinstatePolicy));
      String date = canceledDate.getText();
      date.replaceAll("\n", " ");
      String sysDate = GenericUtils.setDate(testdata.get("Event"),
          Integer.parseInt(testdata.get("noofDays")), Integer.parseInt(testdata.get("noofYear")));
      String futureDate = GenericUtils.setDate(testdata.get("Event"),
          Integer.parseInt(testdata.get("noofDays")), Integer.parseInt(testdata.get("noofYear")));
      String tempDate = date.replaceAll("\n", " "); // Set the date in a particular format
      if (testdata.get("Cancellation type").equals("Cancellation-Cooling off")) {
        System.out.println("Cooling of date is " + tempDate);
        Log.assertThat(tempDate.equalsIgnoreCase("CANCELLED ON " + sysDate),
            "Test case passed successfully", "Test case failed");
      } else if (testdata.get("Cancellation type").equals("Standard Cancellation")) {
        System.out.println("Cooling of date is" + tempDate);
        Log.assertThat(tempDate.equalsIgnoreCase("CANCELLED ON " + futureDate),
            "Test case passed successfully", "Test case failed");
      }
      System.out.println("Test data is missing");
  ...
  public void validatePolicyStatus(HashMap<String, String> testdata, ExtentTest extentedReport)
      throws Exception {
    try {
      WaitUtils.waitForSpinner(driver);
      (new WebDriverWait(driver, 180).pollingEvery(200, TimeUnit.MILLISECONDS)
          .ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
          .withMessage("Unable to find Cancel Policy"))
              .until(ExpectedConditions.elementToBeClickable(tab_viewPolDetails));
      String date = canceledDate.getText();
      String status = date.replaceAll("\\s+", "");
      System.out.println(status);
      Log.assertThat(status.equalsIgnoreCase("ACTIVE"), "Test case passed successfully",
          "Test case failed");
    } catch (Exception e) {
      throw new Exception("Unable to click on Cancel Policy tab :" + e);
    }
