...
public class WaitUtils {
  ...
  public static int maxElementWait = 40;
   ...
   */
	public static boolean waitForListElement(WebDriver driver, List<WebElement> elements, int maxWait) {
		...
		WebDriverWait wait = new WebDriverWait(driver, maxWait);
		try {
			List<WebElement> waitElement = wait.until(ExpectedConditions.visibilityOfAllElements(elements));
			for (WebElement element : waitElement) {
				if (element.isDisplayed() && element.isEnabled()) {
					statusOfElementToBeReturned = true;
					Log.event("List Element is displayed:: " + element.toString());
				}
			}
		} catch (Exception ex) {
			statusOfElementToBeReturned = false;
